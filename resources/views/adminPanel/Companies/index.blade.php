                    ...
                    <div class="card">
                        <div class="card-header d-flex">
                            ...
                            <button type="button" id="add_btn" data-toggle="modal" data-target="#addModal"
                                       class="btn btn-outline-dark btn-lg px-3"><a class="nav-link text-dark" href={{route('admin.companies.create')}}>افزودن<i class="m-1 fa fa-plus"></i>
                                </a>
                        ...
                        <div class="card-body" style="direction: rtl; overflow-x: scroll">
                            <table id="example2" class="table table-bordered table-hover">
                                ...
                                <tbody>
                                @foreach($allCompanies as $items)
                                <tr>
                                    <td>{{$loop->iteration}}</td>
                                    <td>{{$items->companies_name}}</td>
                                    <td>{{$items->companies_summary}}</td>
                                    <td><img width="100px" src="{{ url('') }}{{$items->companies_image}}" alt="{{$items->companies_image}}"></td>
                                    <td>{!! $items->companies_desc !!}</td>
                                    <td>
                                        <a class="btn btn-outline-primary m-1 ajax  " href="{{route('admin.companies.edit', $items->id)}}" data-toggle="tooltip" title="" data-original-title="ویرایش">
                                            <i class="m-1 fa fa-edit"></i>
                                        </a>
                                        <a class="btn btn-outline-danger m-1 ajax  "  href="{{route('admin.companies.destroy', $items->id)}}" data-toggle="tooltip" title="" data-original-title="حذف">
